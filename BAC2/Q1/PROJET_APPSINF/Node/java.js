...
MongoClient.connect("mongodb://localhost:27017",function(err, db){
  ...
  app.get('/connect',function(req,res){//lien vers la page pour ajouter des incidents.
    res.render('compte.html',{connection: req.query.q="se_connecter"});
  });
  ...
  app.get('/search',function(req,res){
    dbm.collection('incidents').find({$or:[{incident:{$regex:req.query.text}},{adresse:{$regex:req.query.text}}]}
      ).toArray((err,doc)=>{
      if (err) throw err;
      res.render('page1.html', {data: doc,utilisateur:req.session.utilisateur});
    })
